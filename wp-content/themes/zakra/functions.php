...
// Add combo product to single product page
function combo_product_action() { 
	global $product;
	global $wpdb;
	$found=false;
	$pid=$product->get_id();
	$combo_name = $wpdb->get_var("SELECT meta_value FROM wp_postmeta WHERE post_id=$pid AND meta_key='_combo'");
	echo '<section class="related products">';
	echo '<h2>Sản phẩm chung bộ</h2>';
	
	if(!empty($combo_name)) {
		echo '<ul class="products columns-4">';
		$products=$wpdb->get_results("SELECT a.post_id,b.post_name,b.post_title,LAY_ANH_SP(a.post_id) as guid, LAY_GIA_SP(a.post_id) as price
									FROM wp_postmeta a
									LEFT JOIN wp_posts b ON b.ID=a.post_id
									WHERE a.meta_key='_combo' AND a.meta_value='$combo_name' and a.post_id <> $pid");
		
		foreach($products as $prod) {
			$img=$prod->guid;
			$src=substr_replace($img, '-300x300', strrpos($img,'.'), 0);
			echo '<li class="zakra-single-article product type-product post-100 status-publish instock product_cat-featured product_cat-gia-treo product_cat-noi-that-phong-ngu has-post-thumbnail featured shipping-taxable purchasable product-type-simple">';
			echo '<a href="'.get_site_url().'/sản phẩm/'.$prod->post_name.'/" class="woocommerce-LoopProduct-link woocommerce-loop-product__link">';
			echo '<img src="'.$src.'" class="attachment-woocommerce_thumbnail size-woocommerce_thumbnail">';
			echo '<h2 class="woocommerce-loop-product__title">'.$prod->post_title.'</h2>';
			echo '<span class="price"><span class="woocommerce-Price-amount amount"><bdi>'.number_format($prod->price, 0, ',', '.').'</bdi><span class="woocommerce-Price-currencySymbol">₫</span></span></span>';
			echo '</a>';
			echo '<a href="?add-to-cart='.$prod->post_id.'" data-quantity="1" class="button product_type_simple add_to_cart_button ajax_add_to_cart" data-product_id="'.$prod->post_id.'" data-product_sku="" rel="nofollow">Thêm vào giỏ hàng</a>';
			echo '</li>';
			$found=true;
		}
		echo '</ul>';
	...
	if(!$found) echo '<span><i>Không có sản phẩm chung bộ</i></span>';
	echo '</section>';
};     
add_action( 'woocommerce_after_single_product_summary', 'combo_product_action', 25 ); 
